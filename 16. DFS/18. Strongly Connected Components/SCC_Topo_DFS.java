...
class SCC_Topo_DFS {
    ...
    static ArrayList<Character> SCC_Sequence = new ArrayList<>();
    ...
    static void SCC() {
        ...
        for (int i = 0; i < node; i++) {
            if (color[i].equals("white")) {
                ...
                Collections.reverse(SCC_Sequence);
                for (int j = 0; j < SCC_Sequence.size(); j++) {
                    System.out.print(" -> " + SCC_Sequence.get(j));
                }
                System.out.println();
                SCC_Sequence.clear();
